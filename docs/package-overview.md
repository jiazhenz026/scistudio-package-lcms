...
| --- | --- | --- | --- | --- |
| app | `ElMaven` | `Collection[Artifact]` (mzML/mzXML) → `LCMSFeatureTable` (`peaks`) | `app_command` (Executable Path), `output_dir` (Save Outputs At) | Interactive launcher: passes the raw files to El-MAVEN as positional arguments via the `prepare_launch` hook so the app opens with them loaded; the user tunes detection and exports a peaks CSV into the output directory, which the block reads back into a standard table. Detection parameters live in El-MAVEN. Needs the El-MAVEN app on the host |
