...
SciStudio R runner via the standard ``inputs`` / ``params`` /
...
from scistudio_blocks_lcms.io.load_mid_table import _detect_sample_columns
from scistudio_blocks_lcms.io.save_table import _to_pandas
...
class AccuCorR(_LCMSBlockMixin, CodeBlock):
    ...
    R script lives at
    ``scistudio_blocks_lcms.external.scripts.accucor`` and is resolved
    ...
    def _resolve_script_path(self, config: BlockConfig) -> str:
        ...
        bundled = resources.files("scistudio_blocks_lcms.external").joinpath("scripts", "accucor.R")
    ...
    def run(
        ...
        * wrap the resulting CSV path in a :class:`MIDTable` with the
        ...
        with tempfile.TemporaryDirectory(prefix="scistudio_accucor_") as tmp_dir:
